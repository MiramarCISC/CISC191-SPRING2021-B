...
public class GameView  extends JPanel implements Runnable, MouseListener
{
    ...
    public GameView()
    {
        ...
        //Used to access head of bullet linked list
        bulletHead = null;
...
//            System.out.println(enemyYPos[y]);
    ...
    //Adds a bullet to bullets Linked List
    ...
    public void paint(Graphics g)
    {
        ...
        //Draws and moves bullets. Removes Bullets outside game view.
        ...
        for (int i = 0; i < alienType1.length; i++){
            for(int j = 0; j < bulletList.size(); j++){
                ...
                if(alienType1[i].getHitBox().intersects(bulletList.get(j).getHitBox())) {
                    alienType1[i].setHit(true);
                    bulletList.remove(bulletList.get(j));
                }
            }

            if(alienType1[i].isHit() || alienType1[i].getY() == GameView_HEIGHT){
                alienType1[i].setY(0);
                alienType1[i].setHit(false);
